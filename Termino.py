...
class Termino:
    ...
    def run(self,conclusion):
        ...
        if conclusion == True:
            ...
            self.pantalla.fill(conclusion.tanque.color)
            Escribir.render_text(self.pantalla, "Juego terminado", (0.37 + ancho_botón / 2, 0.07 - 0.5 * altura_botón + margin_ratio), 60, constantes.BLANCO, "More Sugar")
            Escribir.render_text(self.pantalla, f"Ganador:{conclusion.nombre}" , (0.37 + ancho_botón / 2, 0.17 - 0.5 * altura_botón + margin_ratio), 60, constantes.BLANCO, "More Sugar")
        botones = [
